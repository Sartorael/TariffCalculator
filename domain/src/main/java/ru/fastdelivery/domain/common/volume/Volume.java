 ...
 */
public record Volume(BigDecimal length, BigDecimal width, BigDecimal height) implements Comparable<Volume> {
    ...
    public Volume {
        ...
        if (length.compareTo(BigDecimal.ZERO) < zero || width.compareTo(BigDecimal.ZERO) < zero || height.
                compareTo(BigDecimal.ZERO) < zero) {
            throw new IllegalArgumentException("Parameters must not be negative!");
        }

        length = length.setScale(zero, RoundingMode.HALF_UP).
                divide(BigDecimal.valueOf(scale), zero, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(scale));
        width = width.setScale(zero, RoundingMode.HALF_UP).
                divide(BigDecimal.valueOf(scale), zero, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(scale));
        height = height.setScale(zero, RoundingMode.HALF_UP).
                divide(BigDecimal.valueOf(scale), zero, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(scale));
    ...
    public BigDecimal volumes() {
        ...
        return volume.divide(BigDecimal.valueOf(0.05), RoundingMode.HALF_UP)
