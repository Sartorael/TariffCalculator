...
public record Pack(Weight weight, Volume volume, Departure departure, Destination destination) {
  ...
  private static final Volume maxVolume =
      new Volume(BigDecimal.valueOf(500), BigDecimal.valueOf(500), BigDecimal.valueOf(500));

  public Pack {
    if (weight.greaterThan(maxWeight)) {
      throw new IllegalArgumentException("Package can't be more than " + maxWeight);
    }
    if (volume.length().add(volume.height()).add(volume.width()).compareTo(BigDecimal.valueOf(max))
        > 0) {
