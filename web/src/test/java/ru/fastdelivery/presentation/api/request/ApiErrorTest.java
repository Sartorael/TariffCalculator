...
public class ApiErrorTest {
    ...
    void testRecordMethods() {
        ...
        ApiError differentApiError = new ApiError(HttpStatus.INTERNAL_SERVER_ERROR, "error", "Different message");
        assertThat(apiError).isEqualTo(sameApiError);
        assertThat(apiError).isNotEqualTo(differentApiError);
        assertThat(apiError.toString()).isEqualTo("ApiError[httpStatus=400 BAD_REQUEST, status=error, message=Some error message]");
    }
}
